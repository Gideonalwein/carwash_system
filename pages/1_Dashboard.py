...
if not st.session_state.logged_in:
    st.warning("⚠️ You must login first to access the dashboard.")
    ...
    with filter_cols[0]:
        source_filter = st.selectbox("Sales Type", ["All", "Car Wash", "Drinks"], key="source_filter")

    with filter_cols[1]:
        payment_methods = ["All"] + sorted(sales_df["payment_method"].dropna().unique().tolist())
        payment_filter = st.selectbox("Payment Method", payment_methods, key="payment_filter")
